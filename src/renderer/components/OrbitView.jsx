...
export default function OrbitView({ taskList, activeTask, monitorUpdate, isFocusActive, focusMessages, onBackToDashboard, settings = {} }) {
  const mountRef = useRef(null);
  const [clickedTaskToast, setClickedTaskToast] = useState(null);
  ...
  // Compute scaling multipliers from settings
  const sunScale = (parseFloat(settings.orbitSunSize || 100) / 100);
  ...
  useEffect(() => {
    ...
    // Sun / Central Core scaled by settings
    const baseSunRadius = 2.0 * sunScale;
    const sunGeometry = new THREE.SphereGeometry(baseSunRadius, 24, 24);
    ...
    activeTasks.forEach((task, index) => {
      ...
      const finalPlanetSize = basePlanetSize * planetScale;
      ...
      const planetGeometry = new THREE.SphereGeometry(finalPlanetSize, 16, 16);
    ...
    const handleCanvasClick = (event) => {
      ...
      if (intersects.length > 0) {
        ...
        if (matchingPlanet) {
          // Show 2-second auto-dismiss toast
          setClickedTaskToast(matchingPlanet.taskDetails);
          setTimeout(() => setClickedTaskToast(null), 2000);
          
          // Start focus for clicked task planet
    ...
    const fpsInterval = 1000 / 30; // 30 FPS throttling
  ...
  return (
    <div className="w-full h-full relative orbit-widget-glass rounded-2xl overflow-hidden flex flex-col select-none">
      ...
      {/* Top Header / Drag Handle (Ultra Minimalist: Only Exit Icon) */}
      <div 
        style={{ WebkitAppRegion: 'drag' }}
        className="w-full px-3 py-2 flex items-center justify-between bg-surface-container-high/40 border-b border-white/5 cursor-move"
      >
        <div className="flex items-center gap-1.5 pointer-events-none">
          <span className="material-symbols-outlined text-primary text-base">public</span>
          <span className="text-xs font-bold text-on-surface font-space tracking-wider uppercase opacity-80">Orbit</span>
          {activeTask && (
            <span className="w-2 h-2 rounded-full bg-tertiary animate-pulse ml-1"></span>
          )}
        </div>

        {/* Exit to Console Button (no-drag) */}
      ...
      <div className="flex-1 relative w-full overflow-hidden">
        ...
        {/* Planet Clicked 2-second Auto-Dismiss Toast Pill */}
        {clickedTaskToast && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-20 bg-surface-container/90 backdrop-blur-md border border-primary/40 rounded-xl px-3 py-1.5 shadow-xl flex items-center gap-2 animate-fade-in pointer-events-none">
            <span className="material-symbols-outlined text-primary text-sm">rocket_launch</span>
            <span className="text-xs font-semibold text-on-surface truncate">{clickedTaskToast.title}</span>
            <span className="text-[9px] uppercase font-bold bg-primary/20 text-primary px-1.5 py-0.5 rounded">Focusing</span>
          </div>
        )}

        {/* Ambient Active Task Reminder Pill */}
        {activeTask && !clickedTaskToast && (
